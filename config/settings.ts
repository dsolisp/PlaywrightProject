...
import { URLS, TIMEOUTS, VIEWPORTS } from './constants';

dotenv.config();

// Browser constants (inline to avoid circular dependencies with Playwright config)
const BROWSERS = {
  CHROMIUM: 'chromium',
  FIREFOX: 'firefox',
  WEBKIT: 'webkit',
...
export function settings(): Settings {
  if (!settingsInstance) {
    settingsInstance = {
      ...
      swapiBaseUrl: process.env.SWAPI_BASE_URL || URLS.SWAPI,
      ...
      navigationTimeout: parseInt(
        process.env.NAVIGATION_TIMEOUT || String(TIMEOUTS.NAVIGATION),
        10,
      ),
